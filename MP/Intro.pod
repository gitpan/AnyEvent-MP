...
called C<psub>, which works just like perl's builtin C<sub>:
...
The C<Coro::MP> module is not part of L<AnyEvent::MP>, but a seperate
...
Ports cna be created by attaching a thread to an existing port via
C<rcv_async>, or as here by calling C<port_async> with the code to exeucte
...
That's basically it. Note that every process should ahve some kind of
